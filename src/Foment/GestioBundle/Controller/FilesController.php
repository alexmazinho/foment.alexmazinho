...
class FilesController extends BaseController
{
    ...
    public function exportseccionsAction(Request $request) {
    	...
    	$quotes = $this->queryQuotes($anydades); // array quotes (minimitzar querys)
    	...
    	$query = $this->filtrarArraySeccions($arraySeccions, $queryparams, $anydades);
    	
    	$csvTxt = iconv("UTF-8", "ISO-8859-1//TRANSLIT",implode(";",$header).CRLF);
    	foreach ($query as $seccio) {
    		
    	    $row = array(  $seccio['id'], 
    	                   $seccio['nom'], 
    	                   $anydades, 
                           number_format($seccio['import'], 2, ",", "."),
                	       number_format($seccio['importjuvenil'], 2, ",", "."),
    	                   $seccio['membres'],
                	       $seccio['altesany'],
                	       $seccio['baixesany'],
    	                   number_format($seccio['sumaimports'], 2, ",", "."),
                	       $seccio['rebuts'],
                	       $seccio['quotes'],
    	                   number_format($seccio['sumapagats'], 2, ",", "."),
    	                   number_format($seccio['sumapagats'] - $seccio['sumaimports'], 2, ",", ".")
    	                 );
    ...
    public function domiciliacionsAction(Request $request) {
    	...
    	try {
	    	$ids = explode(",", $request->query->get('facturacio', ''));  // Array ids separats per ,
	    	...
	    	foreach ($ids as $id) {
	    	    ...
	    	    $rebuts = array_merge($rebuts, $facturacio->getRebuts()->toArray());
	    	}
	    	
	    	//if ($facturacio == null) throw new \Exception("No s'ha trobat la facturació ".$id);
	    	//if ($facturacio->getDatafacturacio()->format('Y-m-d') > $datafins->format('Y-m-d')) throw new \Exception("La data màxima ha de ser posterior a la data de facturació ".$facturacio->getDatafacturacio()->format('d/m/Y'));
	    	if (count($rebuts) == 0) throw new \Exception("No s'ha trobat cap rebut a les domiciliacions");
	    	
	    	
	    	//$current = new \DateTime('now');
	    	//$filename = date("Ymd_His") ."_". UtilsController::netejarNom($facturacio->getDescripcio())."_rebuts_fins_".$datafins->format('Ymd').".txt";
	    	$filename = date("Ymd_His") ."_facturacions_".implode("_", $ids)."_rebuts_fins_".$datafins->format('Ymd').".txt";
	    	$fitxer = $ruta.'/'.$filename;
	    	$fs = new Filesystem();
    		...
    		$contents = $this->generarFitxerDomiciliacionsSEPA($rebuts, $datafins, $this->getParameter('ibanfoment'));
    ...
    public function generarFitxerDomiciliacionsSEPA($rebuts, $datafins, $ibanfoment)
    {
        $contents = array();
        //$errors = array();
        ...
        $totalRegistres +=2; // Cabecera de Presentador inicial + Total general
